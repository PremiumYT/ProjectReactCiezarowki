...
import { Container, Row, Col, Card, Form, InputGroup, Button, Modal, ListGroup, Badge } from "react-bootstrap";
...
function FirmPeoplePage() {
    ...
    const handleDetailsOpen = (person) => {
    ...
    return (
        <Container className="mt-5">
            <h1 className="text-center mb-4">Osoby w Firmie</h1>
                    ...
                    <Row className="g-2 align-items-end">
                        <Col xs={6} md={3}>
                            <Form.Group>
                                ...
                                <Form.Control value={filters.imie} onChange={e => setFilters(f => ({ ...f, imie: e.target.value }))} placeholder="Imię" />
                            </Form.Group>
                        </Col>
                        <Col xs={6} md={3}>
                            <Form.Group>
                                <Form.Label>Nazwisko</Form.Label>
                                <Form.Control value={filters.nazwisko} onChange={e => setFilters(f => ({ ...f, nazwisko: e.target.value }))} placeholder="Nazwisko" />
                            </Form.Group>
                        </Col>
                        <Col xs={6} md={3}>
                            <Form.Group>
                                <Form.Label>Stanowisko</Form.Label>
                                <Form.Control value={filters.stanowisko} onChange={e => setFilters(f => ({ ...f, stanowisko: e.target.value }))} placeholder="Stanowisko" />
                            </Form.Group>
                        </Col>
                        <Col xs={6} md={3}>
                            <Form.Group>
                                <Form.Label>Marka ciężarówki</Form.Label>
                                <Form.Control value={filters.marka} onChange={e => setFilters(f => ({ ...f, marka: e.target.value }))} placeholder="Marka" />
                            </Form.Group>
                        </Col>
                    </Row>
                    <Row className="mt-3">
                        <Col className="d-flex justify-content-end">
                            ...
                            <Button variant="dark" size="sm" onClick={() => {}}>Szukaj</Button>
                        </Col>
                    </Row>
                </Card.Body>
            </Card>
            ...
            <Row>
                {filteredPeople.map((p, idx) => (
                    <Col key={idx} xs={12} sm={6} md={4}>
                        <PersonCard person={p} assignedTruck={findAssignedTruck(p)} onDetails={handleDetailsOpen} />
                    </Col>
                ))}
            </Row>
                    ...
                    {selectedPerson && (
                        <>
                            ...
                            <ListGroup variant="flush" className="mt-3">
                                ...
                                <ListGroup.Item><strong>Przypisane ciężarówki:</strong>
                                    <ListGroup className="mt-2">
                                        {companyData.ciezarowki.filter(t => t.przypisanyPracownik === `${selectedPerson.imie} ${selectedPerson.nazwisko}`).map((t, i) => (
                                        ...
                                        {companyData.ciezarowki.filter(t => t.przypisanyPracownik === `${selectedPerson.imie} ${selectedPerson.nazwisko}`).length === 0 && (
